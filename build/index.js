...
└${border}┘`;
    ...
    async step1_EnableDefaultRules() {
        if (this.debugContext.defaultRulesEnabled)
            return;
    ...
    async step2_AnalyzePageLoading() {
        if (!this.debugContext.deviceUrl || !this.page)
            return;
    ...
    async step5_AnalyzeRedirects() {
        this.debugContext.currentStep = "Analyze Redirects";
        // Check network requests for redirects
        const redirects = this.debugContext.networkRequests.filter(req => req.status && req.status >= 300 && req.status < 400);
    ...
    async step6_AnalyzeNetworkAndConsole() {
        ...
        const networkErrors = this.debugContext.networkRequests.filter(req => req.status && req.status >= 400);
    ...
    async step7_PerformCurlAnalysis() {
    ...
    async processLegacyDeviceDebug(input) {
        try {
            ...
            if (!validatedInput.nextThoughtNeeded) {
                ...
                responseData.fullReport = fullReport;
            }
            return {
                content: [{
                        type: "text",
...
const LEGACY_DEVICE_DEBUG_THINKING_TOOL = {
